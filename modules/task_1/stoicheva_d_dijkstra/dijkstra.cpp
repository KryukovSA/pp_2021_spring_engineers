...
int find_unprocessed_point_with_min_distance(const std::vector<int>& graph, std::vector<int>& distances,
...
int process_unprocessed_point(const std::vector<int>& graph, std::vector<int>& distances, 
    ...
    for (size_t point = 0; point < processed.size(); point++) {
        ...
        if (!processed[point] && distance > 0) {
            distances[point] = std::min(distances[point], distances[current_point] + distance);
...
std::vector<int> dijkstra(const std::vector<int>& graph, int start) {
    ...
    while (next_unprocessed_point >= 0) {
        next_unprocessed_point = process_unprocessed_point(graph, distances, processed, next_unprocessed_point);
        if (next_unprocessed_point < 0) {
            next_unprocessed_point = find_unprocessed_point_with_min_distance(graph, distances, processed);
        }
    }
