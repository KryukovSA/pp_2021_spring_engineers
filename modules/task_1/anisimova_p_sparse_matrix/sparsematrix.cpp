...
bool SparseMatrix::operator==(const SparseMatrix& SM) {
    ...
    return values == SM.values && col_indexes == SM.col_indexes && rows == SM.rows;
}
...
SparseMatrix SparseMatrix::operator*(const SparseMatrix& SM) {
    ...
    return SparseMatrix(res_values, res_col_indexes, res_rows, static_cast<int>(res_rows.size()), size);
}
