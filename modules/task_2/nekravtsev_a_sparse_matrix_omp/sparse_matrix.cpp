...
#include <vector>
#include <iostream>
#include "../../../modules/task_2/nekravtsev_a_sparse_matrix_omp/sparse_matrix.h"
...
CCS MultCCSParallel(CCS A, CCS B, int threads) {
  ...
  #pragma omp parallel num_threads(threads)
  {
    ...
    local[thread_num][2].resize(A.size);
    #pragma omp for
    for (int j = 0; j < A.size; j++) {
      int NZ = 0;
      for (int i = 0; i < B.size; i++) {
        ...
        if (fabs(sum) > 0) {
          local[thread_num][0].push_back(sum);
          local[thread_num][1].push_back(i);
          NZ++;
        }
      }
      local[thread_num][2].push_back(NZ + local[thread_num][2][j]);
    }
  }

  for (int i = 0; i < threads; i++) {
    res.value.insert(res.value.end(), local[i][0].begin(), local[i][0].end());
    res.rows.insert(res.rows.end(), local[i][1].begin(), local[i][1].end());
    res.colInd.insert(res.colInd.end(), local[i][2].begin(), local[i][2].end());
  }
